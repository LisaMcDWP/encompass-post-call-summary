...
- `server/activationObjectives.ts` — `pickInteractionForObjective` matches a configured interaction by reading the interaction key from request context (`obj.interactionContextKey || "interaction_key"`) instead of calendar-day distance.
